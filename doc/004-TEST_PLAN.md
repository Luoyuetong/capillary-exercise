...
| **单元测试** | Repository 数据访问 | xUnit + 测试数据库 | 否（用临时 .mdb） |
| **集成测试** | 客户端 ↔ Mock 程序 | 启动真实 Mock，验证通信 | 是（Mock 程序） |
| **端到端测试** | 完整领料流程 | 全部 Mock + 预置数据 | 是（三个 Mock） |
...
| Repository | ≥ 80% |
| 客户端（TcpPlcClient 等） | ≥ 60%（含网络，部分靠集成测试） |
| 整体 | ≥ 75% |
...
**TC-17：TcpPlcClient 连接并执行取料**
- **前置**：MockPLC 已启动
- **操作**：ConnectAsync → FetchFromSlotAsync("A", 5, 10)
- **预期**：连接成功，MockPLC 收到 `CMD:FETCH:A,5,10`，返回 OK
- **对应需求**：FR-08

**TC-18：TcpScannerClient 扫码**
- **前置**：MockScanner 已启动，预置条码 BC001
- **操作**：ScanAsync()
- **预期**：返回 "BC001"
- **对应需求**：FR-09

**TC-19：HttpMesClient 查询类型**
- **前置**：MockMES 已启动，预置 WO001 → CAP-A
...
### 5.1 前置
三个 Mock 全部启动，数据库预置测试数据。

**TC-20：端到端 - 正常领料**
- **前置**：MockMES 预置 WO001→CAP-A；数据库有 CAP-A（BC001, A,5,10）；MockScanner 预置 BC001
- **操作**：界面输入 WO001 / M01 → 开始领料
- **预期**：流程走通，界面显示成功，数据库状态更新，日志记录
- **对应需求**：验收标准 4.1

**TC-21：端到端 - 读码失败**
- **前置**：同上，但 MockScanner 设为"模拟失败"或返回错误条码
- **预期**：劈刀放回，仓位锁定，界面提示读码失败
- **对应需求**：验收标准 4.2

**TC-22：端到端 - MES 拒绝**
- **前置**：同 TC-20，但 MockMES 设为"拒绝上报"
- **预期**：劈刀放回，仓位锁定，界面提示 MES 拒绝
- **对应需求**：验收标准 4.2

**TC-23：端到端 - 无库存**
- **前置**：MockMES 预置 WO002→CAP-B；数据库无 CAP-B
- **预期**：界面提示库存不足，硬件不动作
- **对应需求**：验收标准 4.2
